...
if ($ADMIN->checkEmail($email)) {
    ...
    if ($ADMIN->GenarateCode($email)) {
        $res = $ADMIN->SelectForgetAdmin($email);

        $username = $ADMIN->username;
        $email = $ADMIN->email;
        $resetcode = $ADMIN->restCode;
        ...
        $html .= "<tr><td colspan='3' style='font-size: 14px; padding: 0 25px 10px 25px; color: #666; background-color: #fff; '><b>Adminname: </b> " . $username . "</td></tr>";
